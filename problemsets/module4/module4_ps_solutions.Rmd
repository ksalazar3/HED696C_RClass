...
```{r}
names(df_school_all)
df_school_all %>% select(state_code) %>% str()
df_school_all %>% count(state_code)
df_school_all %>% filter(is.na(state_code)) %>%
...
```{r}
df_school_all %>% select(state_code) %>% str()
df_school_all %>% select(school_type) %>% str()
...
df_school_all %>% count(is.na(num_took_math))
df_school_all %>% filter(is.na(num_took_math)) %>% count()

df_school_all %>% count(is.na(num_prof_math))
...
wwlist <- wwlist %>% 
  mutate(residency=
    case_when(
      ...
      #(is.na(state)) & (is.na(for_country)) ~ NA_character_
    )
  )
