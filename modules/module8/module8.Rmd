...
- To see formatted version of lecture, open `module8.html` or knit and follow along in that version
- you will be able to run code chunks within .Rmd file as usual
- For Lectures 8, 9, and 10 we will go back to `beamer_presentation` with 


### Reading to do before next class

- Work through slides we from Module 8 that we don't get to in class
    - [REQUIRED] slides from section "Tidying data"
    - [OPTIONAL] slides from section "Missing data"
- [REQUIRED] R developer blog on Pivoting
    - [LINK](https://tidyr.tidyverse.org/dev/articles/pivot.html#many-variables-in-column-names)
- [Required] GW chapter 12 (tidy data)
    - Lecture covers this material pretty closely
    - Required 12.1-12.2 & 12.5-12.7
    - Chapter functions for tidying data are limiting and outdated: `gather()` and `spread()`
...
1. Use `group_by` to perform calculations separately within groups and then use `summarise` to create an object with one observation per group. Examples:
    - Creating objects containing summary statistics that are basis for tables and graphs (like Module 5)
    - Creating student-transcript level GPA variable from student-transcript-course level data (focus of Module 7 problem set)
...
```{r}
ipeds_hc_temp <- read_dta(file="https://github.com/ksalazar3/HED696C_Rclass/raw/master/data/ipeds/effy/ey15-16_hc.dta", encoding=NULL) %>% 
...
load(url("https://github.com/ksalazar3/HED696C_Rclass/raw/master/data/nces_digest/nces_digest_table_204_10.RData"))
table204_10
```
...
#library(haven)
#library(labelled)
ipeds_hc_all <- read_dta(file="https://github.com/ksalazar3/HED696C_Rclass/raw/master/data/ipeds/effy/ey15-16_hc.dta", encoding=NULL)
```
