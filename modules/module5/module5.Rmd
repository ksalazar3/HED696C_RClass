---
title: "Module 6: Augmented vectors,  factor + labelled class"
subtitle:  ""
author: 
date: 
fontsize: 8pt
...
df_event %>% filter(event_type == "public hs", univ_id == 110635) %>% 
  mutate(pct_fr_lunch = (fr_lunch/total_students_pub)*100) %>% 
...
1. What is the min value of `med_inc` across all events?  
    ...
    - Name the new variable you are creating "min_med_income"

1. What is the mean value of `fr_lunch` across all events?  
    - Hint: Use mean(), don't forget to use `na.rm = TRUE`
    - Name the new variable you are creating "mean_fr_lunch"
...
__Example__: Count the number of events across all rows (all universities); then within groups of `instnm` (for each university)
...
- Task: For recruiting events by UC Berkeley, count number of recruiting events by event_type
```{r, results="hide"}
df_event %>% count(instnm)
...
event_berk %>% group_by(event_type) %>% 
  summarise(
    n_events = n(),
    n_miss_fr_lunch = sum(is.na(fr_lunch)),
...
Use one code chunk for this exercise. Tackle this one step at a time, building your code by adding piped operations for each step, and then run the entire code chunk when you have answered all parts of this question. Create your own variable names.  
...
- if _input_ has any missing values (`NA`), than output will be missing. Missing values are contagious!
...
1. Using the `event_berk` object, group by `instnm`, `event_inst`, & `event_type`.  
    1. Create vars for number non_missing for these racial/ethnic groups (`pct_white_zip`, `pct_black_zip`)  
...
__Task__: Using object `event_berk`, create a new object `gt50p_lat_bl` with the following measures for each combination of `event_type` and `event_inst`:
