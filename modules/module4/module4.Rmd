...
- __If you're interested in completing the class asynchronously, email me for approval__
...
- This common recruitment tool reinforces bias in admissions practices
  ...
  - [The Institute for College Access and Success](https://ticas.org/student-success/series-of-reports-find-common-recruiting-practice-perpetuate-bias-but-crucial-for-college-access-of-underrepresented-students/)
...
- We collected these data using FOIA requests; out team's data management skills were key to the success of this project! 
...
__Task__: Using object `wwlist`, print data for "first-gen" prospects (`firstgen`) for the following selected variables: `state`, `hs_city`, `sex` [output omitted]
...
```{r, results="hide"}
wwlist_temp <-wwlist %>% filter(firstgen == "Y") 
```
...
#create variable and check variable all in one step [but don't overwrite original df]
